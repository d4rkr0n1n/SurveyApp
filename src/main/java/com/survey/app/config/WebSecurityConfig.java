...
public class WebSecurityConfig {
	...
	public UserDetailsService userDetailsService() {
		...
		manager.createUser(
				User.withUsername("user").password(passwordEncoder().encode("redhat")).roles("USER").build());
		manager.createUser(
				User.withUsername("admin").password(passwordEncoder().encode("redhat")).roles("ADMIN").build());
		return manager;
